...
#define SC_MIN -2 * M_PI // most accurate representation of pi, use to bound the functions
...
{
  bool s, c, t, e, a = false;  // -s -c -t -e user option to individually perform and display sin,
  int ch = 0;                  //  cos, tin, exp, respectively, -a will perform them all
  if((ch = getopt(argc, argv, OPTIONS)) != -1)  // parse user input for options provided
  { 
...
void Sin()
{
  ...
  for(double i = SC_MIN; i <= (SC_MAX + SCT_STEP); i += SCT_STEP)
  {
    ...
    sin_lib = sin(i);  // the actual sin function from math.h lib, use as comparison
...
void Cos()
{
  ...
  for(double i = SC_MIN; i <= (SC_MAX + SCT_STEP); i += SCT_STEP)
  {
    ...
    cos_lib = cos(i);  // the actual cos function from math.h lib, use as comparison
...
void Tan()
{
  ...
  for(double i = TAN_MIN; i <= TAN_MAX; i += SCT_STEP)
  {
    ...
    tan_lib = tan(i);  // the actual tan function from math.h lib, use as comparison
...
void Exp()
{
  ...
  while(total_num < 91)  // there are total of 91 numbers between 0 and 9 if stepping by 0.1
  {
  ...
  static double input_num = EXP_MIN;  // number starting off from 0 to reach 9
   ...
   for(double k = 1.0; (fabs(term) > EPSILON); k+= 1.0)  // ensure a limited decimals result
   {
    term = (input_num/k) * term;  // current term in series
    sum += term;
   }
   exp_lib = exp(input_num);  // actual exp function from math.h lib, use as comparision
